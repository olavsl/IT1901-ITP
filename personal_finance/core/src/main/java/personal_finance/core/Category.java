...
public class Category {
  ...
  private double calcTotalMonth(LocalDate startDate, List<Transaction> transactions) {
    ...
    for (Transaction transaction : transactions) {
      if (transaction.getDate().isAfter(startDate.minusDays(1)) && transaction.getDate().isBefore(endDate)
          && transaction.getCategory().getTitle() == this.getTitle()) {
  ...
  /**
   * Sums up all transactions with date between and including 1st day of month to
   * last of month and checks if sum is <= limit
   * 
  ...
  /**
   * Sums up all transactions with date between and including 1st day of month to
   * last of month and calculates the remaining limit
   * 
   * @param startDate    The date to start the calculation
   * @param transactions The list of all transactions from user
