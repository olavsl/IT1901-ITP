...
public class BudgetController extends SceneSwitcher {
  ...
  @FXML
  private DatePicker budgetStartDate;
  @FXML
  private TextField categoryLimit;
  @FXML
  private TableColumn<Category, String> categoryTitles;
  @FXML
  private TableColumn<Category, Double> categoryLimits;
  @FXML
  private TableColumn<Button, Double> deleteBtn;
  @FXML
  private TableView<Category> categoryOverview;
  @FXML
  ...
  public void handleAddCategory(ActionEvent event) {
    ...
    for (Category c : this.user.getBudget().getCategories()) {
      if (c.getTitle().equals(title)) {
        ...
        // throw new IllegalArgumentException("A category with that name already
        // exists!");
        return;
      }
    }
